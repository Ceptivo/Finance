...
function AddAccountDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  ...
  const reset = () => { setName(""); setType("bank"); setBalance(""); setCurrency("ZAR"); setColor(COLORS[0]); setIsLiability(false); };
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              ...
              <Select value={currency} onValueChange={setCurrency}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
                  {["ZAR","USD","EUR","GBP","AUD","CAD"].map((c) => <SelectItem key={c} value={c}>{c}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
          </div>
